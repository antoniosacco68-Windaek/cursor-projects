...
try {
    ...
    $command.CommandText = "SELECT [Mittente], [NomeMittente], [Destinatario], [Oggetto], [Corpo], [CC], [CCN], [FormatoHTML], [Allegato] FROM [dbo].[EmailData] WHERE [ID] = @Id"
    ...
    $updateCmd.CommandText = "UPDATE [dbo].[EmailData] SET [Inviata] = 1, [DataInvio] = GETDATE(), [EmailID] = @EmailID WHERE [ID] = @Id"
    ...
    try {
        if ($connection -and $connection.State -eq [System.Data.ConnectionState]::Open) {
            ...
            $errorUpdateCmd.CommandText = "UPDATE [dbo].[EmailData] SET [Note] = @ErrorMsg WHERE [ID] = @Id"
