-- ==============================================================================
-- QUERY EVENTI PNEUMATICI FINALE - ANTERIORI + POSTERIORI 
-- Basata sulla logica CORRETTA di "Originale Eventi Pneumatico solo Anteriore ok.sql"
-- ==============================================================================
...
WITH AnagraficaPneumaticiCompleta AS (
    ...
    WHERE primo_deposito.RankDeposito = 1
      AND NOT EXISTS (
          ...
          WHERE sl_check.S_IdVeicolo = primo_deposito.D_IdVeicolo 
            AND asl_check.Art_Codice = primo_deposito.D_ArtCodice
...
-- Cronologia per eventi (ANTERIORI + POSTERIORI)
...
EventiSequenziali AS (
    SELECT 
        ...
        -- ULTIMI PNEUMATICI IN DEPOSITO (per rimontaggi)
...
-- 1. MONTAGGI (tutti i pneumatici montati da ArtSchedaLavoro)
SELECT 
    ...
    'Montaggio pneumatico ' + ap.Posizione AS Note__c
...
-- 2. SMONTAGGI ANTERIORI (pneumatici messi in deposito)
SELECT 
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    ...
    es.Id_Articoli AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    CASE 
        WHEN es.D_TipoDepositoR1 LIKE '%Deposito%' THEN 'Smontaggio per deposito stagionale (anteriore)'
        WHEN es.D_TipoDepositoR1 LIKE '%finite%' THEN 'Smontaggio per deposito finite (anteriore)'
        WHEN es.D_TipoDepositoR1 LIKE '%Smaltite%' THEN 'Smontaggio per smaltimento (anteriore)'
        WHEN es.D_TipoDepositoR1 LIKE '%Porta Via%' THEN 'Smontaggio porta via cliente (anteriore)'
        ELSE 'Smontaggio per ' + ISNULL(es.D_TipoDepositoR1, 'deposito') + ' (anteriore)'
    END AS Note__c
FROM EventiSequenziali es
INNER JOIN AnagraficaPneumaticiCompleta ap ON ap.IdVeicolo = es.IdVeicolo 
    AND ap.CodiceArticolo = es.PneumaticoSmontato
    AND ap.Posizione = 'ANTERIORE'
WHERE es.PneumaticoSmontato IS NOT NULL

UNION ALL

-- 3. SMONTAGGI POSTERIORI (pneumatici messi in deposito)
SELECT 
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    CAST(ap.External_Id__c + '_SMONT_' + FORMAT(es.Data_Lavori, 'yyyyMMddHHmmss') AS VARCHAR(255)) AS External_Id__c,
    'Smontaggio' AS Tipo__c, 
...
FROM EventiSequenziali es
INNER JOIN AnagraficaPneumaticiCompleta ap ON ap.IdVeicolo = es.IdVeicolo 
    AND ap.CodiceArticolo = es.PneumaticoSmontatoPost
    AND ap.Posizione = 'POSTERIORE'
WHERE es.PneumaticoSmontatoPost IS NOT NULL

UNION ALL

-- 4. RIMONTAGGI ANTERIORI (servizi stagionali)
SELECT 
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    ...
    es.IdSchedaLavoro AS [Scheda_di_lavoro__r:Commessa__c:External_ID__c],
    es.Id_Articoli AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    'Rimontaggio da deposito - cambio stagionale (anteriore)' AS Note__c
FROM EventiSequenziali es
INNER JOIN AnagraficaPneumaticiCompleta ap ON ap.IdVeicolo = es.IdVeicolo 
    AND ap.CodiceArticolo = es.UltimoPneumaticoInDeposito
    AND ap.Posizione = 'ANTERIORE'
WHERE es.ServizioStagionale = 1 AND es.UltimoPneumaticoInDeposito IS NOT NULL

UNION ALL

-- 5. RIMONTAGGI POSTERIORI (servizi stagionali)
SELECT 
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    CAST(ap.External_Id__c + '_RIMONT_' + FORMAT(es.Data_Lavori, 'yyyyMMddHHmmss') AS VARCHAR(255)) AS External_Id__c,
    'Rimontaggio' AS Tipo__c, 
...
FROM EventiSequenziali es
INNER JOIN AnagraficaPneumaticiCompleta ap ON ap.IdVeicolo = es.IdVeicolo 
    AND ap.CodiceArticolo = es.UltimoPneumaticoInDepositoPost
    AND ap.Posizione = 'POSTERIORE'
