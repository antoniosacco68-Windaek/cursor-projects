...
'    End If
    ...
    ' --- INVIO EMAIL TRAMITE SQL SERVER ---
    ...
    cmd.CommandText = "SP_InvioEmailPS" ' Nome della Storage procedure modificata
    ...
    cmd.Parameters.Append _
        cmd.CreateParameter("@Body", adVarChar, adParamInput, 4000, TTextBody)
    cmd.Parameters.Append _
        cmd.CreateParameter("@Attachment", adVarChar, adParamInput, 400, strAttachment2)
    
    ' Esegue la stored procedure e ottiene il recordset con i risultati
    Dim rs As ADODB.Recordset
    Set rs = cmd.Execute
    
    ' Ora la stored procedure ritorna immediatamente
    ...
    If Not rs.EOF Then
        ...
        emailID = rs.Fields("EmailID").Value
    Else
        emailInviata = False
    ...
    ' Visualizza messaggio e apre il form di monitoraggio
    If emailInviata Then
        ' Apri il form di monitoraggio dell'email
        MsgBox "L'email è stata messa in coda per l'invio. Puoi monitorare lo stato nella form di monitoraggio.", vbInformation
        
        ' Apri la form di monitoraggio
        DoCmd.OpenForm "frmMonitorEmail", , , , , acDialog, emailID
