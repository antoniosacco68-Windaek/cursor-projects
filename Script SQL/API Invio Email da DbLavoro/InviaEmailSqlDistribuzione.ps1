...
# Carica l'assembly System.Web per HttpUtility
...
try {
    ...
    $connectionString = "Server=localhost;Database=PiattaformeWeb;Trusted_Connection=True;Connection Timeout=30;Pooling=true;Min Pool Size=5;Max Pool Size=100;"
    ...
    $command.CommandText = "SELECT [Mittente], [NomeMittente], [Destinatario], [Oggetto], [Corpo], [CC], [CCN], [FormatoHTML], [Allegato] FROM [dbo].[EmailDataDistribuzione] WHERE [ID] = @Id"
    ...
    # Codifica il subject per supportare caratteri speciali
    [void]$contentBuilder.AppendLine("Subject: =?utf-8?Q?$([System.Web.HttpUtility]::UrlEncode($Oggetto).Replace("+", " ").Replace("%", "="))?=")
    ...
    if ($hasAttachments) {
        ...
        [void]$contentBuilder.AppendLine("Content-Transfer-Encoding: quoted-printable")
    } else {
    ...
    if ($hasAttachments) {
        ...
        if ($isHtml) {
            ...
            # Codifica il corpo in quoted-printable per gestire i caratteri speciali
            $encodedBody = [System.Web.HttpUtility]::UrlEncode($Corpo).Replace("+", " ").Replace("%0D%0A", "`r`n").Replace("%", "=")
            [void]$contentBuilder.AppendLine($encodedBody)
        ...
        # Codifica il corpo in quoted-printable per gestire i caratteri speciali
        $encodedBody = [System.Web.HttpUtility]::UrlEncode($Corpo).Replace("+", " ").Replace("%0D%0A", "`r`n").Replace("%", "=")
        [void]$contentBuilder.Append($encodedBody)
    }
    ...
    $updateCmd.CommandText = "UPDATE [dbo].[EmailDataDistribuzione] SET [Inviata] = 1, [DataInvio] = GETDATE(), [EmailID] = @EmailID WHERE [ID] = @Id"
    ...
    try {
        if ($connection -and $connection.State -eq [System.Data.ConnectionState]::Open) {
            ...
            $errorUpdateCmd.CommandText = "UPDATE [dbo].[EmailDataDistribuzione] SET [Note] = @ErrorMsg WHERE [ID] = @Id"
