    ...
    Do While Not PR_Documenti.EOF
        ...
        If PR1.RecordCount > 0 Then
            ...
            Do While Not PR1.EOF
                ...
                ' METODO: Modifica il RecordSource del report per filtrare il record specifico
                Call StampaReportConFiltro(ReportDaStampare, "IdFatture", IdCorrente, NomePdf)
        ...
        Else
            ...
            If PR1.RecordCount > 0 Then
                ...
                Do While Not PR1.EOF
                    ...
                    ' METODO: Modifica il RecordSource del report per filtrare il record specifico
                    Call StampaReportConFiltro(ReportDaStampare, "ID", IdCorrente, NomePdf)
                    ContatorePDF = ContatorePDF + 1
...
' Funzione per stampare il report con un filtro specifico
Sub StampaReportConFiltro(NomeReport As String, CampoFiltro As String, ValoreFiltro As Long, PercorsoPDF As String)
    
    On Error GoTo ErroreStampa
    Dim rpt As Report
    Dim RecordSourceOriginale As String
    
    ' Apri il report in modalità design per modificare il RecordSource
    DoCmd.OpenReport NomeReport, acViewDesign
    Set rpt = Reports(NomeReport)
    
    ' Salva il RecordSource originale
    RecordSourceOriginale = rpt.RecordSource
    
    ' Modifica il RecordSource per filtrare solo il record desiderato
    If InStr(RecordSourceOriginale, "WHERE") > 0 Then
        ' Se c'è già una clausola WHERE, aggiungi AND
        rpt.RecordSource = RecordSourceOriginale & " AND " & CampoFiltro & " = " & ValoreFiltro
    Else
        ' Se non c'è WHERE, aggiungila
        rpt.RecordSource = RecordSourceOriginale & " WHERE " & CampoFiltro & " = " & ValoreFiltro
    End If
    
    ' Chiudi e riapri il report con i nuovi dati
    DoCmd.Close acReport, NomeReport, acSaveNo
    
    ' Stampa il PDF
    DoCmd.OutputTo acOutputReport, NomeReport, acFormatPDF, PercorsoPDF, False
    
    ' Ripristina il RecordSource originale
    DoCmd.OpenReport NomeReport, acViewDesign
    Set rpt = Reports(NomeReport)
    rpt.RecordSource = RecordSourceOriginale
    DoCmd.Close acReport, NomeReport, acSaveYes
    
    Exit Sub
    
ErroreStampa:
    MsgBox "Errore nella stampa del report: " & Err.Description
    
    ' Prova a ripristinare il report se aperto
    If CurrentProject.AllReports(NomeReport).IsLoaded Then
        DoCmd.Close acReport, NomeReport, acSaveNo
    End If
End Sub
