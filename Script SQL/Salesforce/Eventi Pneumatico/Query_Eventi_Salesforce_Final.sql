-- ==============================================================================
-- EVENTI PNEUMATICO COMPLETI - ANTERIORI + POSTERIORI DA DEPOSITO
-- ==============================================================================

WITH AnagraficaPneumaticiCompleta AS (
    -- PNEUMATICI MONTATI (ANTERIORI)
    SELECT 
    ...
    WHERE primo_deposito.RankDeposito = 1
      AND NOT EXISTS (
          ...
          WHERE sl_check.S_IdVeicolo = primo_deposito.D_IdVeicolo AND asl_check.Art_Codice = primo_deposito.D_ArtCodice
            ...
            AND sl_check.Data_Lavori < sl_primo.Data_Lavori  -- Solo se montaggio è PRECEDENTE al deposito
      )
    ...
    WHERE primo_deposito_post.RankDeposito = 1
      AND NOT EXISTS (
          ...
          WHERE sl_check.S_IdVeicolo = primo_deposito_post.D_IdVeicolo AND asl_check.Art_Codice = primo_deposito_post.D_ArtCodicePost
            AND asl_check.Art_Fascia IN ('A','B','C','U','R')
            AND sl_check.Data_Lavori < sl_primo_post.Data_Lavori  -- Solo se montaggio è PRECEDENTE al deposito
      )
),

-- Cronologia per eventi ANTERIORI E POSTERIORI
...
-- RISULTATO: EVENTI COMPLETI (ANTERIORI + POSTERIORI)
-- 1. MONTAGGI ANTERIORI
SELECT DISTINCT
    ...
    MIN(es.Id_Articoli) AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    'Montaggio pneumatico ANTERIORE' AS Note__c
...
GROUP BY ap.External_Id__c, es.Data_Lavori, es.Km, es.IdSchedaLavoro, es.PneumaticoMontato
...
SELECT DISTINCT
    ...
    MIN(es.Id_Articoli) AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    CASE 
        WHEN es.D_TipoDepositoR1 LIKE '%Deposito%' THEN 'Smontaggio ANTERIORE per deposito stagionale'
        WHEN es.D_TipoDepositoR1 LIKE '%finite%' THEN 'Smontaggio ANTERIORE per deposito (finite)'
        WHEN es.D_TipoDepositoR1 LIKE '%Smaltite%' THEN 'Smontaggio ANTERIORE per smaltimento'
        WHEN es.D_TipoDepositoR1 LIKE '%Porta Via%' THEN 'Smontaggio ANTERIORE porta via cliente'
        ELSE 'Smontaggio ANTERIORE per ' + ISNULL(es.D_TipoDepositoR1, 'deposito')
    END AS Note__c
FROM EventiSequenziali es
...
GROUP BY ap.External_Id__c, es.Data_Lavori, es.Km, es.IdSchedaLavoro, es.PneumaticoSmontato, es.D_TipoDepositoR1

UNION ALL

-- 3. RIMONTAGGI ANTERIORI
SELECT DISTINCT
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    ...
    es.IdSchedaLavoro AS [Scheda_di_lavoro__r:Commessa__c:External_ID__c],
    MIN(es.Id_Articoli) AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    'Rimontaggio ANTERIORE da deposito (cambio stagionale)' AS Note__c
FROM EventiSequenziali es
...
GROUP BY ap.External_Id__c, es.Data_Lavori, es.Km, es.IdSchedaLavoro, es.UltimoPneumaticoInDeposito

UNION ALL

-- 4. SMONTAGGI POSTERIORI (SOLO DA DEPOSITO)
SELECT DISTINCT
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    CAST(ap.External_Id__c + '_SMONT_' + FORMAT(es.Data_Lavori, 'yyyyMMddHHmmss') AS VARCHAR(255)) AS External_Id__c,
    'Smontaggio' AS Tipo__c, 
    ...
    MIN(es.Id_Articoli) AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    CASE 
        WHEN es.D_TipoDepositoR2 LIKE '%Deposito%' THEN 'Smontaggio POSTERIORE per deposito stagionale'
        WHEN es.D_TipoDepositoR2 LIKE '%finite%' THEN 'Smontaggio POSTERIORE per deposito (finite)'
        WHEN es.D_TipoDepositoR2 LIKE '%Smaltite%' THEN 'Smontaggio POSTERIORE per smaltimento'
        WHEN es.D_TipoDepositoR2 LIKE '%Porta Via%' THEN 'Smontaggio POSTERIORE porta via cliente'
        ELSE 'Smontaggio POSTERIORE per ' + ISNULL(es.D_TipoDepositoR2, 'deposito')
    END AS Note__c
FROM EventiSequenziali es
...
GROUP BY ap.External_Id__c, es.Data_Lavori, es.Km, es.IdSchedaLavoro, es.PneumaticoSmontatoPost, es.D_TipoDepositoR2

UNION ALL

-- 5. RIMONTAGGI POSTERIORI (SOLO DA DEPOSITO)
SELECT DISTINCT
    ap.External_Id__c AS [Pneumatico__r:Pneumatico__c:External_Id__c],
    CAST(ap.External_Id__c + '_RIMONT_' + FORMAT(es.Data_Lavori, 'yyyyMMddHHmmss') AS VARCHAR(255)) AS External_Id__c,
    'Rimontaggio' AS Tipo__c, 
    ...
    es.IdSchedaLavoro AS [Scheda_di_lavoro__r:Commessa__c:External_ID__c],
    MIN(es.Id_Articoli) AS [Riga_scheda_di_lavoro__r:ArticoliSchedaLavoro__c:External_ID__c],
    'Rimontaggio POSTERIORE da deposito (cambio stagionale)' AS Note__c
FROM EventiSequenziali es
...
GROUP BY ap.External_Id__c, es.Data_Lavori, es.Km, es.IdSchedaLavoro, es.UltimoPneumaticoInDepositoPost
